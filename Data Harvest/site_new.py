class site_new:
	...
	def add_program(self, ws):
		self.programs.append(program_new(ws))
...
class program_new:
	def __init__(self, ws):
		...
		self.telephony_platform = add_element(ws['D49'])
		self.tele_users = self.get_users(ws, 56)
		self.cc_platform = add_element(ws['D63'])
		self.agent_types = add_element(ws['E66'])
		self.agents = self.get_users(ws, 72)
	...
	def get_complexity(self, ws):
		answers = (add_element(ws['E78']).lower(), \
					add_element(ws['E77']).lower(), \
					add_element(ws['E76']).lower(), \
	...
	def get_reporting(self, ws):
		answers = (add_element(ws['K79']).lower(), \
					add_element(ws['K80']).lower(), \
					add_element(ws['K77']).lower(), \
					add_element(ws['K78']).lower())
